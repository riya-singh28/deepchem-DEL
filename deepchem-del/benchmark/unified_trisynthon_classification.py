...
def denoise_data(file_path: str,
    ...
    data = calculate_poisson_enrichment(df=data_unique,
                                        control_cols=control_cols,
                                        target_cols=target_cols)
    target_threshold = calculate_hit_threshold(
        df=data, column_name='Poisson_Enrichment', percentile=percentile)
    data['hits'] = (data['Poisson_Enrichment'] > target_threshold).astype(int)
    data.to_csv(output_file_path, index=False)
    return output_file_path


def unified_trisynthon_classification_pipeline(
